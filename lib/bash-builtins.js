...
/**
 * Commande cd - Change de répertoire avec support de cd -
 * @param {Array} args - Arguments de la commande
 * @param {Object} context - Contexte (fileSystem, currentPath, setCurrentPath, saveFileSystem, variables)
 */
export function cmdCd(args, context) {
    const { fileSystem, currentPath, setCurrentPath, saveFileSystem, variables } = context;
    ...
    if (arg === '-') {
        ...
        if (!fileSystem[previousPath]) {
            showError(`cd: ${previousPath}: Dossier introuvable`);
            return;
        }
        
        if (fileSystem[previousPath].type !== 'dir') {
            showError(`cd: ${previousPath}: N'est pas un dossier`);
            return;
        }
