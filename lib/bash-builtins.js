...
export function cmdCd(args, context) {
    const { fileSystem, getCurrentPath, setCurrentPath, variables } = context;
    ...
    if (args.length === 0) {
        ...
        targetPath = currentUser.home;
    } else if (args[0] === '-') {
        // cd - = retourner au répertoire précédent
        targetPath = variables.OLDPWD || currentPath;
        addLine(targetPath); // bash affiche le nouveau répertoire avec cd -
    } else {
    ...
    if (!fileSystem[targetPath]) {
        errorFn(`bash: cd: ${args[0] || '~'}: Aucun fichier ou dossier de ce type`);
        return;
    }
    
    // Vérifier que c'est un répertoire
    if (fileSystem[targetPath].type !== 'dir') {
        errorFn(`bash: cd: ${args[0] || '~'}: N'est pas un répertoire`);
        return;
    }
