 ...
 * @param {Object} context - Contexte contenant currentPath et variables
...
export function getEnvironmentVariables(context) {
    ...
    const currentPath = context.getCurrentPath ? context.getCurrentPath() : (context.currentPath || '/');
...
/**
 * Initialise OLDPWD dans les variables du contexte
 * @param {Object} context - Contexte d'exécution
 */
export function initOldPwd(context) {
    if (!context.variables) {
        context.variables = {};
    }
    
    if (!context.variables.OLDPWD) {
        context.variables.OLDPWD = context.currentPath || '/';
    }
}

/**
 * Met à jour OLDPWD lors d'un changement de répertoire
 * @param {Object} context - Contexte d'exécution
 * @param {string} newPath - Nouveau chemin
 * @param {string} oldPath - Ancien chemin
 */
export function updatePaths(context, newPath, oldPath) {
    if (!context.variables) {
        context.variables = {};
    }
    
    context.variables.OLDPWD = oldPath;
    // PWD est mis à jour automatiquement par getEnvironmentVariables()
}
