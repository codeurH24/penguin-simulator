...
export function parseCommandLine(command) {
    ...
    // NOUVELLE ÉTAPE : Expand les braces après le parsing initial
    return expandBraces(parts);
}
...
export function handleOutputRedirection(context, redirections) {
    ...
    const captureAddLine = (text) => {
        ...
        if (!fileSystem[filePath]) {
            showError(`Le fichier n'existe pas`);
            return;
        }
...
/**
 * Expand les braces bash du type {a,b,c} en plusieurs arguments
 * @param {Array} parts - Tableau des parties parsées
 * @returns {Array} - Tableau avec les braces expandées
 */
export function expandBraces(parts) {
    const result = [];
    
    for (const part of parts) {
        if (part.includes('{') && part.includes('}')) {
            const expanded = expandSingleBrace(part);
...
/**
 * Expand une seule partie contenant des braces
 * @param {string} str - Chaîne contenant des braces comme "project/src/{js,css}"
 * @returns {Array} - Tableau des variantes expandées
 */
function expandSingleBrace(str) {
    // Trouver la première paire de braces
    ...
    const prefix = str.substring(0, openBrace);
    const suffix = str.substring(closeBrace + 1);
    const braceContent = str.substring(openBrace + 1, closeBrace);
    
    // Séparer les options par la virgule
    const options = braceContent.split(',').map(opt => opt.trim());
    
    // Créer toutes les combinaisons
    const results = [];
    for (const option of options) {
        const combination = prefix + option + suffix;
        
        // Récursion si il y a encore des braces dans le résultat
        if (combination.includes('{') && combination.includes('}')) {
            results.push(...expandSingleBrace(combination));
        } else {
