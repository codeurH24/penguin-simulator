...
/**
 * Expanse les wildcards dans les arguments
 * @param {Array} args - Arguments potentiellement avec wildcards
 * @param {Object} fileSystem - Système de fichiers pour trouver les correspondances
 * @param {string} currentPath - Répertoire courant pour la résolution des chemins
 * @returns {Array} - Arguments avec wildcards expandés
 */
export function expandWildcards(args, fileSystem, currentPath) {
    return args.flatMap(arg => {
        // Si l'argument ne contient pas de wildcards, le retourner tel quel
        if (!arg.includes('*') && !arg.includes('?')) {
            return [arg];
        }
        
        // Rechercher les fichiers correspondant au pattern
        const matches = findMatchingFiles(arg, fileSystem, currentPath);
        
        // Si aucune correspondance trouvée, garder l'argument original
        if (matches.length === 0) {
            return [arg];
        }
...
function findMatchingFiles(pattern, fileSystem, currentPath) {
    const matches = [];
    ...
    Object.keys(fileSystem).forEach(filePath => {
        if (filePath.startsWith(currentDir)) {
            const fileName = filePath.substring(currentDir.length);
            
            // Vérifier que c'est un fichier direct (pas dans un sous-répertoire)
...
function matchesWildcard(fileName, pattern) {
    // Gestion spéciale pour les fichiers cachés
